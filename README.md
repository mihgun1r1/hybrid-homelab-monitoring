...
[![Homepage](https://img.shields.io/badge/Homepage-007ACC?style=for-the-badge&logo=visualstudiocode&logoColor=white)](https://gethomepage.dev/)
...
```text
[ Homepage Dashboard ] (Port 80)
                                  │
┌─────────────────────────────────┼─────────────────────────────────┐
▼                                 ▼                                 ▼
[ Web File Storage ]      [ Observability Stack ]           [ Identity Services ]
File Browser (:8080)      Grafana (:3000)                   Samba 4 AD DC (:389)
        │                         ▲                                 │
        ▼ (Storage watch)         │ (Scrapes)                       ▼ (LDAP query)
Auto-Organizer (Python)   Prometheus (:9090) ◄────────────── AD Exporter (:9150)
                                  │
                                  ▼ (Alerts)
                           Alertmanager (:9093) ──► AI Responder ──► Telegram
```

* **Homepage Portal (`homepage`):** Unified entry gateway running on port 80 with Docker socket integration displaying real-time service health badges and system utilization.
...
* **Storage & Auto-Organizer (`filebrowser`, `auto_organizer`):** Web file management UI backed by an event-driven background daemon (`watchdog`) that scans ingest directories and sorts files by MIME type (documents, media, archives, code). Features file-transfer size validation (preventing race conditions during active uploads)... and collision avoidance through incremental file versioning.
...
| :--- | :--- | :--- | :--- |
| **Homepage** | `80` | HTTP | Centralized homelab services gateway and status dashboard |
...
* **Homelab Portal:** Open [http://localhost](http://localhost) to access all core services via Homepage.
...
* **Reset Password:**
  ```bash
  docker compose exec samba_ad samba-tool user setpassword it_user --newpassword="NewPassword2026!"
  ```
